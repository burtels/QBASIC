...
srcUrl = "puzzles/steve-debug.bas";

defaultSourcePath = srcUrl;
        if (appMode == "play") {
...
if (srcUrl) {
    ...
    if (!res.ok) {
        alert("Could not load source file: " + srcUrl + " (" + res.status + ")");
        return;
    }
...
async function _reloadSource() {
    if (!defaultSourcePath) {
        alert("No source file configured.");
        return;
    }

    if (!confirm("Discard current edits and reload the original source file?")) {
        return;
    }

    var res = await fetch(defaultSourcePath);
    if (!res.ok) {
        alert("Could not load source file: " + defaultSourcePath + " (" + res.status + ")");
        return;
    }

    var decoder = new TextDecoder("iso-8859-1");
    var qbcode = decoder.decode(await res.arrayBuffer());
...
this.reloadSource = _reloadSource;
